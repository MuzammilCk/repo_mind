...
from services.tool_executor import execute_tool  # Functional import
...
class ExecutionPlan(BaseModel):
    """Complete execution plan generated by Gemini"""
    reasoning: str = Field(description="Strategy for solving the query")
    tool_calls: List[ToolCall] = Field(
        description="Ordered list of tools to execute",
        max_length=20  # Safety limit
    )
    expected_outcome: str = Field(description="What the final report should contain")
...
Be autonomous - make decisions without asking. Use thinking to plan. Self-correct if tools fail."""
    ...
    def marathon_agent(
        ...
        """
        Local-First Marathon Agent (Macro-Planning).
        Reduces API calls from 15+ to 2-3 to bypass Rate Limits.
        """
        print(f"🏃 Marathon Agent (Local-First) - Goal: {user_goal}")
        
        try:
            # STEP 1: PLAN (1 API Call)
            plan = self._generate_execution_plan(user_goal)
            
            # STEP 2: EXECUTE LOCALLY (0 API Calls)
            execution_results = self._execute_plan_locally(plan)
            
            # STEP 3: SYNTHESIZE (1 API Call)
            final_report = self._synthesize_results(user_goal, plan, execution_results)
    ...
    def _generate_execution_plan(self, query: str) -> ExecutionPlan:
        """Generate execution plan using Gemini 3 Generate API (Stateless)"""
        print("🧠 Generating plan...")
        try:
            # Use client.models.generate_content for structured output
            # Note: Using config=Types.GenerateContentConfig logic implicitly via dict
            response = self.client.models.generate_content(
                model=self.gemini_model, 
                contents=f"Query: {query}\n\nPlan the execution using available tools (search_github_repos, analyze_repository, etc).",
                config={
            ...
            # In google-genai SDK, parsed response is in .parsed if schema is provided
            ...
            return ExecutionPlan(
                ...
                tool_calls=[ToolCall(tool_name="search_github_repos", arguments={"query": query}, purpose="Fallback Search")],
                expected_outcome="Basic results"
            )

    def _execute_plan_locally(self, plan: ExecutionPlan) -> List[Dict[str, Any]]:
        """Execute tools locally with Adaptive Course Correction"""
        results = []
        queue = list(plan.tool_calls) # Mutable queue
        executed_count = 0
        
        print(f"📋 Executing plan with {len(queue)} steps...")
        
        while queue and executed_count < 15: # Safety limit
            call = queue.pop(0)
            executed_count += 1
            print(f"🔧 Step {executed_count}: {call.tool_name} - {call.purpose}")
            
            try:
                # Direct functional call to tool_executor
                result = execute_tool(call.tool_name, call.arguments)
                
                # ADAPTIVE LOGIC: Check for "soft failures" (e.g. empty results)
                needs_replan = False
                replan_reason = ""
                ...
                if isinstance(result, dict):
                    if "count" in result and result["count"] == 0:
                        needs_replan = True
                        replan_reason = f"Tool returned 0 results. Args: {call.arguments}"
                    if "status" in result and result["status"] == "error":
                        needs_replan = True
                        replan_reason = f"Tool execution error: {result.get('error')}"
                elif not result:
                     needs_replan = True
                     replan_reason = "Tool returned empty result"

                if needs_replan:
                    print(f"   ⚠️ Failure Detected: {replan_reason}")
                    print("   🔄 Triggering Adaptive Course Correction...")
                    
                    # Call API to get new steps
                    new_steps = self._replan(call, replan_reason)
                    
                    if new_steps:
                        print(f"   ✅ Course Corrected: Added {len(new_steps)} new steps")
                        # Prepend new steps to queue to execute immediately
                        for step in reversed(new_steps):
                            queue.insert(0, step)
                    else:
                        print("   ❌ Re-planning yielded no solution. Continuing...")

                results.append({
                    "tool": call.tool_name,
                    "args": call.arguments,
                    "result": result,
                    "success": not needs_replan # Mark as 'issue' if we had to replan
                })
    ...
    def _replan(self, failed_step: ToolCall, error_context: str) -> List[ToolCall]:
        """
        Adaptive Course Correction Agent.
        Called when local execution hits a roadblock.
        """
        prompt = f"""EXECUTION FAILURE:
Failed Step: {failed_step.tool_name}
Args: {failed_step.arguments}
Context: {error_context}

TASK: Generate 1-2 new tool calls to fix this.
Example: If search returned 0, try a broader query.
Return ONLY the new steps."""
        
        try:
            # Use same "Plan" schema but for a small fix
            response = self.client.models.generate_content(
                 model=self.gemini_model,
                 contents=prompt,
                 config={
                    "response_mime_type": "application/json",
                    "response_schema": ExecutionPlan, # Reuse schema
                    "temperature": 0.4 
                }
            )
            
            if hasattr(response, 'parsed') and response.parsed:
                return response.parsed.tool_calls
            
            fix_plan = ExecutionPlan.model_validate_json(response.text)
            return fix_plan.tool_calls
        except Exception as e:
            print(f"   ⚠️ Re-planning failed: {e}")
            return []

    def _synthesize_results(self, query: str, plan: ExecutionPlan, results: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        evidence = []
        for r in results:
            result_str = json.dumps(r.get('result', {}), default=str)
            if len(result_str) > 5000: # Truncate large results
                result_str = result_str[:5000] + "... [truncated]"
            evidence.append(f"--- Tool: {r['tool']} ---\nArgs: {r.get('args')}\nResult: {result_str}\n")
            
        context = "\n".join(evidence)
        
        prompt = f"""Goal: {query}
        
Execution Plan Strategy: {plan.reasoning}
        
Evidence Collected:
{context}
        
Synthesize a comprehensive answer based on the evidence.
Include a 'Summary', 'Key Findings', and 'Code Suggestions' if applicable."""
